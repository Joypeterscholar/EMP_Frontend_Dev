...
- Frontend: `docs/object-ref-contract.md` (this file)
- Backend: `docs/object-ref-contract.md`
...
fields on a tag (see `src/components/TagModelForm.jsx`, which reads them off
the `objectRef` computed in `SceneComponent.jsx`). `position` and `camera`
keep travelling inside the existing `taggedInfo` JSON blob that the frontend
already sends - there was no need to duplicate them as separate fields to
fix the machine-identification problem this contract exists for.
...
- Frontend: [`src/utils/objectRef.js`](../src/utils/objectRef.js)
  (`buildObjectPath` + `computeObjectId`), using the Web Crypto API
  (`crypto.subtle.digest`) so no extra dependency is needed.
- Backend (`Environmental-Mapping-EMP-` repo): `src/utils/object-ref.ts`
  (same functions, same fingerprint string and truncation), using Node's
  `crypto` module.
...
## What the frontend resolves before hashing

`src/utils/objectRef.js` handles the three things this session needs to work
out:
- **Readable path** - `buildObjectPath` walks `mesh.parent` up to the root,
  joining each node's name with `/`.
- **`_primitive0` / `_primitive1` pieces** - `resolveMachineNode` detects the
  `_primitiveN` suffix exporters add when they split one visual machine into
  several mesh nodes, and hashes the shared parent instead, so every piece
  of the same machine resolves to the same `objectId`.
- **Instanced meshes** - an instance is just another node in the hierarchy,
  so it is told apart from its source (and from other instances) the same
  way as any other sibling: by `objectPath`, via the sibling index. No
  special-case code is needed because Babylon instances share the source
  mesh's vertex count and material, so only the path differs.
...
To tag "the machine" rather than one primitive, `resolveMachineNode` walks up
to the parent node - `Building A/Room 1/WasteBin[0]` - and computes the
`objectId` from the parent's own path/point count/material. `meshName` for a
primitive tag stays as the primitive's raw name (`WasteBin_primitive0`) so it
can still be traced back during debugging.
...
- A throwaway test proves the same id comes back twice for the same input:
  - Frontend: [`scripts/verify-object-id.mjs`](../scripts/verify-object-id.mjs)
    (`node scripts/verify-object-id.mjs`).
  - Backend: `src/tests/object-ref.test.ts` in the backend repo (`npm test`).
